...
#             logger.info(f"Checking if topic selection message needs to be sent to {phone_number.id}")
#             ############## Control Group ##############
#             if 'control' in phone_number.arm.name.lower():
#                 logger.info(f"Assigning topic number {week_num} to {phone_number.id}")
#                 try:
...
#                 #continue
#             ############## Intervention Group ##############
#             if 'ai_chat' in phone_number.arm.name.lower():
#                 message_tracker_queryset = MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)
#                 if message_tracker_queryset.exists():
#                     if message_tracker_queryset[0].sent_topic_selection_message:
...
#                             except IntegrityError:
#                                 logger.error(f"IntegrityError when trying to create WeeklyTopic object for {phone_number.id}")
#                             SendMessage.objects.create(phone_number=phone_number, message=message, route='outgoing_scheduled_topic_selection',
#                                                     week_num=week_num,picklist=picklist_json,current_weekday=current_weekday,
#                                                     default_topic_id=default_topic_id,message_tracker_col='sent_topic_selection_message')
...
#         ########## Goals ##########
#         if current_weekday == 2 and 'ai_chat' in phone_number.arm.name.lower():
#             logger.info(f"Checking if goals message needs to be sent to {phone_number.id}")
#             message_tracker_queryset = MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)
#             if message_tracker_queryset.exists():
#                 if message_tracker_queryset[0].sent_goal_message:
#                     logger.info(f'Skipping {phone_number.id} because goal message already sent for week {week_num}')
#                     #continue
#                 else:
#                     message, picklist_json = get_goals_message(phone_number,week_num)
#                     logger.info(f"message for {phone_number.id} is {message}")
#                     if message:
#                         SendMessage.objects.create(phone_number=phone_number, message=message, route='outgoing_scheduled_goal',
#                                                 week_num=week_num,picklist=picklist_json,current_weekday=current_weekday,
#                                                 default_topic_id=None,message_tracker_col='sent_goal_message')

#         ########## Goals Feedback ##########
#         if current_weekday == 5 and 'ai_chat' in phone_number.arm.name.lower():
#             logger.info(f"Checking if goals feedback message needs to be sent to {phone_number.id}")
#             message_tracker_queryset = MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)
#             if message_tracker_queryset.exists():
#                 if message_tracker_queryset[0].sent_goal_feedback_message:
#                     logger.info(f'Skipping {phone_number.id} because goal feedback message already sent for week {week_num}')
#                     #continue
#                 else:
#                     message, picklist_json = get_feedback_message(phone_number,week_num)
#                     logger.info(f"message for {phone_number.id} is {message}")
#                     if message:
#                         SendMessage.objects.create(phone_number=phone_number, message=message, route='outgoing_goal_feedback',
#                                                 week_num=week_num,picklist=picklist_json,current_weekday=current_weekday,
#                                                 default_topic_id=None,message_tracker_col='sent_goal_feedback_message')

#         ########## final pilot message ##########
#         if week_num == 9 and current_weekday == 1:
#             logger.info(f"Checking if final pilot message needs to be sent to {phone_number.id}")
#             if phone_number.final_pilot_message_sent:
#                 logger.info(f'Skipping {phone_number.id} because final pilot message already sent for week {week_num}')
#                 #continue
#             else:
#                 message, include_name = get_final_message(phone_number)
#                 logger.info(f"message for {phone_number.id} is {message}")
#                 if message:
#                     SendMessage.objects.create(phone_number=phone_number, message=message, route='outgoing_final_pilot_message',
#                                             week_num=week_num,picklist=None,current_weekday=current_weekday,
#                                             default_topic_id=None,message_tracker_col=None,include_name=include_name)
                
#         ########## Scheduled info ##########
#         logger.info(f"Checking if scheduled info message needs to be sent to {phone_number.id}")
#         message_tracker_queryset = MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)
#         if message_tracker_queryset.exists():
#             logger.info("Message tracker exists")
#             message_tracker_col = f'sent_info_message_{current_weekday}'
#             if getattr(message_tracker_queryset[0], message_tracker_col):
#                 logger.info(f'Skipping {phone_number.id} because scheduled info message already sent for week {week_num}')
#                 #continue
#             else:
#                 message, include_name, picklist_json, message_tracker_col = send_scheduled_message(phone_number,week_num,current_weekday)
#                 logger.info(f"message for {phone_number.id} is {message}")
#                 if message:
#                     SendMessage.objects.create(phone_number=phone_number, message=message, route='outgoing_scheduled_info',
#                                                 week_num=week_num,picklist=picklist_json,current_weekday=current_weekday,
#                                                 default_topic_id=None,message_tracker_col=message_tracker_col,include_name=include_name)
#     return None
