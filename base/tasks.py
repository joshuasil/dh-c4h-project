...
def handle_topic_selection(phone_number, week_num, current_weekday):
    message_tracker, _ = MessageTracker.objects.get_or_create(phone_number=phone_number, 
                            week_no=week_num, defaults={'sent_topic_selection_message': False})
    logger.info(f"message tracker created for {phone_number.id} and week {week_num}")
...
def handle_scheduled_info_messages(phone_number, week_num, current_weekday):
    ...
    if not getattr(message_tracker, message_tracker_col, False):
