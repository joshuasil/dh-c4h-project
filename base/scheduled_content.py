...
def send_topic_selection_message():
    ...
    for phone_number in phone_numbers:
        ...
        if week_num > int(4):
            logger.info(f'Skipping {phone_number.phone_number} because it is week {week_num}')
            continue
        if current_weekday != int(1):
            logger.info(f'Skipping {phone_number.phone_number} because it is weekday {current_weekday}')
            continue
        if phone_number.sub_group:
            topic_num = week_num*2
        else:
            topic_num = week_num*2-1
        logger.info(f"Assigning topic number {topic_num} to {phone_number.phone_number}")
        WeeklyTopic.objects.create(phone_number=phone_number, topic_id=topic_num, week_number=week_num)
        logger.info(f"Updated message tracker for {phone_number.phone_number}")
    ...
    for phone_number in phone_numbers:
        try:
            ...
            week_num,current_weekday = get_week_num_andcurrent_weekday(phone_number.created_at)
            logger.info(f"Current hour for {phone_number.phone_number}: {week_num}")
            ...
            try:
                if MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)[0].sent_topic_selection_message:
                    logger.info(f'Skipping {phone_number.phone_number} because topic selection message already sent for week {week_num}')
                    continue
            except:
                pass
            ...
            logger.error(f"Error sending topic selection message to {phone_number.phone_number}: {e}")
    ...
    for phone_number in phone_numbers:
        try:
            if not phone_number.active:
                continue
            week_num,current_weekday = get_week_num_andcurrent_weekday(phone_number.created_at)
            logger.info(f"Current week for {phone_number.phone_number}: {week_num}")
            ...
            if getattr(message_tracker, message_tracker_col):
                logger.info(f'Skipping {phone_number.phone_number} because scheduled info message was sent for week {week_num} and day {current_weekday}')
                continue
            ...
            logger.error(f"Error sending scheduled message to {phone_number.phone_number}: {e}")
    ...
    for phone_number in phone_numbers:
        try:
            if not phone_number.active:
                continue
            week_num,current_weekday = get_week_num_andcurrent_weekday(phone_number.created_at)
            logger.info(f"Current weekday for {phone_number.phone_number}: {week_num}")
            if week_num <= 0 or week_num > int(4):
                logger.info(f'Skipping {phone_number.phone_number} because it is week {week_num}')
                continue
            if current_weekday != int(2):
                logger.info(f'Skipping {phone_number.phone_number} because it is not day {current_weekday}')
                continue
            if MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)[0].sent_goal_message:
                logger.warning(f'Skipping {phone_number.phone_number} because goal message sent for week {week_num}')
                continue
            weekly_topic_id = WeeklyTopic.objects.filter(phone_number=phone_number,week_number=week_num)[0].topic_id
            ...
            logger.info(f"Sent goals message to {phone_number.phone_number} for topic {weekly_topic_id}")
            picklist_json = json.dumps(picklist)
    ...
    for phone_number in phone_numbers:
        try:
            ...
            if MessageTracker.objects.filter(phone_number=phone_number,week_no=week_num)[0].sent_goal_feedback_message:
                logger.warning(f'Skipping {phone_number.phone_number} because goal feedback message sent for week {week_num}')
                continue
            weekly_topic_id = WeeklyTopic.objects.filter(phone_number=phone_number,week_number=week_num)[0].topic_id
            topic_goals = TopicGoal.objects.filter(topic_id=weekly_topic_id)
            if not topic_goals:
            ...
            logger.error(f"Error sending goal feedback message to {phone_number.phone_number}: {e}")
...
def send_final_pilot_message():
    ...
    for phone_number in phone_numbers:
        try:
            ...
            logger.error(f"Error sending final message to {phone_number.phone_number}: {e}")